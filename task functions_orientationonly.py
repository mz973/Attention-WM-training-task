...
def trialGen_ori(c,limit=None):   #randomize bar position   #0<c<2; d2 <1 (max is 180 degree), limit is the max orientation a distractor has
    triallist=[] #triallist: x1, y1,x2,y2
    p = []; C=[]
    d2 = np.arange(0.05,0.45,0.05) #only works when c > max(d2)
    for y in d2:
        y1 = np.array([(-1*c-y)/2,(c-y)/2, round(np.random.uniform(-0.4,0.4),2)]) #first 2 elements allows the same c, 3rd one doesn't
        y2 = y1+y
        x1 = np.round(np.random.uniform(0.5,0.9,3),1) 
        x2 = np.round(np.random.uniform(0.5,0.9,3),1)  * np.random.choice((1,-1),3)
...
#    p, parameters = stimulirule(c,0.45) #0.5 = +/- 90 degree
    c, p, parameters = trialGen_ori(0.8)
    triallist=[]
